...
class DaoPrazoDemanda extends DaoPrazo {
    ...
    public static function getPrazoExtensao($prazo = false) {
    	try {
    		...
    		$sql = 'select p.sq_prazo, p.nu_proc_dig_ref, p.nu_proc_dig_res, p.id_usuario_destino, 
       				  ...
       				  p.dt_resposta, p.fg_status, p.tx_resposta, p.tx_solicitacao, p.id_unidade_usuario_resposta,
    				  e.id, e.nu_proc_dig_ref_pai, coalesce(e.ha_vinculo, false) as ha_vinculo,
    				  coalesce(e.legislacao_situacao, 0) as legislacao_situacao,
    				  e.legislacao_descricao, e.dt_minuta_resposta, e.id_prazo_pai
    ...
    public static function getPrazoResposta($seqPrazo = false) {
    	try {
    		...
    		$sql = "SELECT cp.sq_prazo, trim(ext.nu_proc_dig_ref_pai) AS digital_pai, cp.nu_proc_dig_ref AS nu_ref, 
    					...
    					usr.nome AS nm_usuario_resposta, unr.nome AS nm_unidade_resposta,
			...
			if (trim($prazo->prazo->acoes, ',') != '') {
	        	$sql = 'insert into snas.tb_prazo_vinculo_ppa_acoes(id_prazo, codigo_orgao, codigo_programa, codigo_acao, exercicio)
						...
						left join snas.tb_prazo_vinculo_ppa_acoes v on
							(v.id_prazo = :prazo and v.st_ativo = 1
	        				and v.codigo_orgao=a."codigoOrgao"
							and v.codigo_programa=a."codigoPrograma"
							and v.codigo_acao=a."codigoAcao" 
							and v.exercicio=a.exercicio)
	        	...
	        	$arrAcoes = split(',', trim($prazo->prazo->acoes, ','));
	        	foreach ($arrAcoes as $acao) {
