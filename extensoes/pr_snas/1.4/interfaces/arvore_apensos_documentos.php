...
include(__BASE_PATH__ . '/extensoes/pr_snas/1.4/classes/Arvore.php');
include("function/auto_load_statics.php");
include("interfaces/detalhar_documentos.php");

$urlManager = __APPMODELS__ . "arvores/listar_apensos_documentos.php";
$digital = $_GET['digital'];
$arvore = new Arvore();

$vinculacao = new Vinculacao();
$root = $vinculacao->setDocumentoRoot($digital, 2/* Apensos */);
$arvore->setRootId($root);
$elementos = $arvore->getVinculacaoDocumento($root, $urlManager, 2/* Apensos */);
?>
    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function() {
                treeOperations = new TreeOperations($('#tree-apensos-documentos'), '<?php print($urlManager); ?>');

                /*Arvore */
                $('#tree-apensos-documentos').simpleTree({
                    animate: true,
                    autoclose: false,
                    ...
                    afterContextMenu: function(element, event) {
                        ...
                        if (className.indexOf('root') >= 0) {
                            $('#menu-tree-apensos-documentos .edit, #menu-tree-apensos-documentos .delete').hide();
                            $('#menu-tree-apensos-documentos .expandAll, #menu-tree-apensos-documentos .collapseAll').show();
                        }
                        else {
                            $('#menu-tree-apensos-documentos .expandAll, #menu-tree-apensos-documentos .collapseAll').hide();
                        }
                        $('#menu-tree-apensos-documentos').css('top', event.pageY).css('left', event.pageX).show();

                        $('*').click(function() {
                            $('#menu-tree-apensos-documentos').hide();
                        });
                    }
                ...
                treeOperations.expandAll($('#tree-apensos-documentos > .root > ul'));

                /*Menus*/
                $('#menu-tree-apensos-documentos .expandAll').click(function() {
                    treeOperations.expandAll($('#tree-apensos-documentos > .root > ul'));
                });
                $('#menu-tree-apensos-documentos .collapseAll').click(function() {
                    treeOperations.collapseAll();
                });

                $('#menu-tree-apensos-documentos .expandAll').append(langManager.expandAll);
                $('#menu-tree-apensos-documentos .collapseAll').append(langManager.collapseAll);

            });
        </script>
    </head>
    <body>
        <div class="contextMenu" id="menu-tree-apensos-documentos">
        ...
        <ul id="tree-apensos-documentos" class="arvoreDocumentos">
			<?php
