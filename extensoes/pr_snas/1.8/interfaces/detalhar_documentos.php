...
include_once(__BASE_PATH__ . '/extensoes/pr_snas/1.8/interfaces/pesquisar_documentos.php');

require_once __BASE_PATH__ . '/extensoes/pr_snas/1.8/interfaces/dialog_exportar_dados.php';

require_once __BASE_PATH__ . '/extensoes/pr_snas/1.8/interfaces/dialog_responder_prazo.php';

require_once (__BASE_PATH__ . '/extensoes/pr_snas/1.8/interfaces/form_novos_prazos_multiplos.php');
...
<html>
<head>
	<script	src="<?php echo '/extensoes/pr_snas/1.8/javascripts/documentos.js'?>"></script>
	<script type="text/javascript">
		var boxOrigemOpen = false;
		var boxDestinoOpen = false;
		$(document).ready(function() {

			$('#btnFiltroDemanda').click(function() {
				carregarListaPrazos();
			});
...
<?php			}?>
			...
			/* Exportar Dados */
			$('#botao-exportar-documento').click(function() {
				exportarDados($('#ID_DETALHAR_DOCUMENTO').val(), $('#DIGITAL_DETALHAR_DOCUMENTO').val());
			});
...
?>
			<div style="margin-top: 10px" id="tabsGridDemandas">
 
				<div id="divFiltroDemandas" style="text-align:left; padding:5px;" class="ui-widget-header ui-corner-all">
					Filtrar Demandas:
	  				<span class="conteudo" style="margin-left:3px; margin-right:10px;">
	  					<select id="selFiltroOrigemDemanda" class="FUNDOCAIXA1" style="width:auto;">
							<option value="TD">Todas as Demandas</option>
	  						<option value="PU">Para a Unidade</option>
	  						<option value="EN">Encaminhadas</option>
	  						<option value="GU">Geradas pela Unidade</option>
	  					</select>
	  				</span>
	  				Status:
					<span class="conteudo" style="margin-left:3px; margin-right:10px;">
	  					<select id="selFiltroStatusDemanda" class="FUNDOCAIXA1" style="width:auto;">
	  						<option value="TD">Todas as Demandas</option>
	  						<option value="AR">Para Resposta</option>
	  						<option value="MT">Em Minuta</option>
	  						<option value="RP">Respondidas</option>
	  					</select>
	  				</span>
	  				<button id="btnFiltroDemanda" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only">
...
?>
	...
	<div id="box-filtro-tramite" class="box-filtro">
		<div class="row">
			<label>Destino:</label>
			<div class="conteudo">
				<input type="text" id="FILTRO_TRAMITE" onKeyUp="DigitaLetraSeguro(this)" class="FUNDOCAIXA1">
			</div>
		</div>
	</div>
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$('#botao-filtro-tramite').click(function() {
				$('#box-filtro-tramite').dialog('open');
			});
			...
			$("#FILTRO_TRAMITE").autocompleteonline({
				url: 'modelos/combos/lista_tramite.php',
				idComboBox: 'LISTA_TRAMITE'
			});
			/*Remover*/
			...
			$('#botao-adicionar-tramite').click(function() {
				if (!$('#LISTA_TRAMITE').val()) {
					return false;
				}
				$('#container-tramites').append(tmpl.replace('%d', - 1).replace('%d', $('#LISTA_TRAMITE').val()).replace('%s', $('#LISTA_TRAMITE option:selected').text()));
			});
		});
	</script>
</body>
