<?php
require_once __BASE_PATH__ . '/extensoes/pr_snas/1.8/classes/DaoPrazoDemanda.php';
...
try {
	if ($lista == 'ppa') {
		...
		$strTabAcoes = '<table class="display" id="tblAcoes"><thead><tr>
							<th class="'.$strClasseTh.'">Programa</th>
							<th class="'.$strClasseTh.'" style="width: 20%;">A&ccedil;&atilde;o</th>
							<th class="'.$strClasseTh.'" style="width: 20%;">Localizador</th>
							<th class="'.$strClasseTh.'" style="width: 20%;">Plano Or&ccedil;ament&aacute;rio</th>
							<th class="'.$strClasseTh.'" style="width: 8%;">Exerc&iacute;cio</th>
							<th class="'.$strClasseTh.'" style="width: 8%;">Op&ccedil;&otilde;es</th>
						</tr></thead>';
		...
		if ($arrDados && ($numTam > 0)) {
			for ($i=0;$i<$numTam;$i++) {
				...
				$strTabAcoes .= '<td class="tdLeft">' . quebrarTexto($arrDados[$i]['ACAO'], 30) . '</td>';
				$strTabAcoes .= '<td class="tdLeft">' . quebrarTexto($arrDados[$i]['LOCALIZADOR'], 30) . '</td>';
				$strTabAcoes .= '<td class="tdLeft">' . quebrarTexto($arrDados[$i]['P_O'], 30) . '</td>';
		...
		echo '<div id="tabsPPA'.$idPrazo.'"><ul>';
		echo '<li><a href="#divTabMetas'.$idPrazo.'">Objetivos / Metas</a></li>';
		echo '<li><a href="#divTabAcoes'.$idPrazo.'">A&ccedil;&otilde;es</a></li>';
		echo '</ul>';
		echo '<div id="divTabMetas'.$idPrazo.'">' . $strTabMetas . '</div>';
		echo '<div id="divTabAcoes'.$idPrazo.'">' . $strTabAcoes . '</div>';
		echo '</div>';
		
		echo '<script type="text/javascript">';
		echo '$("#tabsPPA'.$idPrazo.'").tabs({ active: 1 });';
		echo '</script>';
