		...
		function(data) {
			try {
				...
				if (data) {
					...
					$('#pLocalizador').html(data.COD_LOCALIZADOR + ' - ' + data.DES_LOCALIZADOR);
					$('#pPO').html(data.COD_PLANO_ORCAMENTARIO + ' - ' + data.TIT_PLANO_ORCAMENTARIO);
					...
					$('#divAcao').show();
	            	$('#divMetasAcoes').dialog('open');
	            	$('#divMetasAcoes').dialog('option', 'title', 'Detalhes da Ação / Localizador / PO');
                } else {
                	alert('Não foi encontrado a meta vinculada!');
                }
			} catch (e) {
...
<div id="divMetasAcoes" title="Dados SIOP">
	<div id="divTexto">
		...
		<div id="divAcao">
			...
			<p id="pLocalizador"></p>
			<label>Plano Or&ccedil;ament&aacute;rio:</label>
			<p id="pPO"></p>
