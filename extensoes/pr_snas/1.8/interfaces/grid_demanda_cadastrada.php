	...
	$(document).ready(function() {
		...
		oTableDocumentosDemanda = $('#TabelaDocumentosDemanda').dataTable({
			...
			fnRowCallback: function(nRow, aData, iDisplayIndex) {
				...
				$('td:eq(0)', nRow).html('');
				if (prazoDaUnidade) {
					$('td:eq(0)', nRow).html('<input type="checkbox" idPrazo="' + aData[0] + '" dataPrazo="' + aData[8] + '" class="chkPrazo" />');
				}
				...
				if (aData[4] != '') {
					$('td:eq(4)', nRow).html(jquery_toggle(aData[4], 100));
				}
				
				/* Converter formato Date para String (dd/mm/aaaa) */
				$('td:eq('+colData+')', nRow).html(convertDateToString(aData[8]));
...
<table class="display" id="TabelaDocumentosDemanda">
	<thead>
		<tr>
			...
			<th class="style13 column-assunto">Grupo</th>
