...
//						($('#TRAMITAR_PARA_CADASTRAR_SOLICITACOES_GENERICA').val() != '0') &&
			...
			function jquery_cadastrar_solicitacao_generica() {
				...
				if (confirm('Você tem certeza que deseja cadastrar esta demanda?')) {
					...
					var extras = {PRIORIDADES: 0, TRAMITES:1};
					extras.PRIORIDADES = {id: 0, id_campo: 1};
					extras.TRAMITES = {id:0, id_campo:1};
					extras.PRIORIDADES.id = new Array();
					extras.PRIORIDADES.id_campo = new Array();
					extras.TRAMITES.id = new Array();
					extras.TRAMITES.id_campo = new Array();
					...
					$('#container-tramites span select option').each(function(){
						$(this).each(function() {
							extras.TRAMITES.id.push($(this).attr('value'));
							extras.TRAMITES.id_campo.push($(this).attr('id_campo'));
						});
					});

					$.post('extensoes/pr_snas/1.8/modelos/generico/cf.php', {
						...
						ID_UNID_CAIXA_ENTRADA: $('#LISTA_TRAMITE').val(),
						DESTINO: $('#LISTA_TRAMITE :selected').text(),
						SOLICITACAO: $('#OCR_CADASTRAR_SOLICITACOES_GENERICA').val(),
						PRIORIDADE: $('#LISTA_PRIORIDADE').val(),
						extras: extras,
	...
	<body>
		...
		<div class="div-form-dialog" id="div-form-cadastrar-solicitacoes-generica">
			<form id="solicitacao_sic_generico" enctype="multipart/form-data" method="POST" action="modelos/generico/adicionar.php">
				...
				<div class="row-tramites">
					<div class="row">
						<label>*TRAMITAR PARA:</label>
						<span class="conteudo">
							<select class="FUNDOCAIXA1" id="LISTA_TRAMITE" ></select>
						</span>
						<img title="Adicionar" class="botao-auxiliar-fix-combobox" id="botao-adicionar-tramite" src="imagens/fam/add.png">
						<img title="Filtrar" class="botao-auxiliar-recuo-fix-combobox" id="botao-filtro-tramite" src="imagens/fam/application_edit.png">
					</div>
					<div class="row" id="container-tramites"></div>
				</div>
