		...
		$('#divDadosSiop').dialog({
	        ...
	        buttons: {
				Selecionar: {id: "btnSelecionar",
					text: "Selecionar", 
					click: function() {
						if (($('#hdnIdMetasSelecionadas').val() != '') || ($('#hdnIdAcoesSelecionadas').val() != '')) {
							if (confirm('Você tem certeza que deseja selecionar as metas e/ou ações?')) {
								$.post("modelos/prazos/prazos.php", {
										...
										acoes: $('#hdnIdAcoesSelecionadas').val()
									},
									function(data) {
										try {
											if (data.success == 'true') {
						                        alert('Metas e/ou Ações Selecionadas com Sucesso!');
						                        carregarMetasPPA(); /* ATUALIZA A LISTA DE METAS E AÇÕES NA TELA DE RESPOSTA */
...
<div id="divDadosSiop" title="Dados SIOP">
	...
	<input type="hidden" id="hdnIdAcoesSelecionadas" />
