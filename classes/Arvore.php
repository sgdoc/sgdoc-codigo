        ...
        try {
            ...
            if (!empty($out)) {
                foreach ($out as $key => $value) {
                    ...
                    $ausente = (!Processo::validarProcessoAreaDeTrabalho($value['FILHO'])) ? array('classe' => 'ausente', 'title' => 'Este processo nao esta na sua área de trabalho.', 'ausente' => 'true') : array('classe' => '', 'title' => '', 'ausente' => 'false');
                    $idElemento = str_replace(array('.', '/', '-'), array('', '', ''), $value['FILHO']);
                    if (true) {
                        $supp = "<ul class='ajax'>"
                                . "<li id='$idElemento'>{url:'{$pageName}?action=getElementList&ownerEl={$value['FILHO']}',idElemento:'{$idElemento}',stAusente:{$ausente['ausente']}}</li>"
