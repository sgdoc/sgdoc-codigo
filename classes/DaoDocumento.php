...
class DaoDocumento {
    ...
    public static function uniqueDocumento(Documento $documento) {
        ...
        try {
            ...
            // checar se já existe um documento idêntico a este
            $stmt = Controlador::getInstance()->getConnection()->connection->prepare("
                SELECT count(*) as num FROM TB_DOCUMENTOS_CADASTRO WHERE NUMERO = ? AND ORIGEM = ? AND TIPO = ? AND ID <> ?
            ");
            $stmt->bindParam(1, $documento->documento->numero, PDO::PARAM_STR);
            $stmt->bindParam(2, $documento->documento->origem, PDO::PARAM_STR);
            ...
            if ($out !== false && $out['NUM'] > 0) {
                ...
                return new Output(array('success' => 'false', 'error' => 'Já existe documento deste tipo e origem com este número'));
            } else {
            ...
            return new Output(array('success' => 'false', 'error' => 'Erro ao tentar verificar duplicidade de documento'));
        }
    }
    ...
    public static function alterarDocumento(Documento $documento) {
        try {
            ...
            //Trata Prioridade vazia ou não informada
            $prioridade = $documento->documento->prioridade;
            $prioridadeType = PDO::PARAM_INT;
            if (($prioridade == 'null') || (strlen(trim($prioridade)) == 0)) {
                $prioridade = NULL;
