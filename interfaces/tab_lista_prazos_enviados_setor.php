...
<script type="text/javascript">
    ...
    var oTableEnviadosSetor = null;

    $(document).ready(function() {
        $('#tabs-prazos-env-set').click(function() {
            oTableEnviadosSetor.fnDraw(false);
        });

        /*DataTable*/
        oTableEnviadosSetor = $('#tabela_prazos_enviados_setor').dataTable({
            ...
            sAjaxSource: "modelos/prazos/listar_prazos_enviados_setor.php",
            oLanguage: {
            ...
            fnServerData: function(sSource, aoData, fnCallback) {
                $.getJSON(sSource, aoData, function(json) {
                    ...
                    if (json.iTotalRecords == 0) {
                        jquery_datatable_complementa_mensagem_vazia('tabela_prazos_enviados_setor');
                    }
                });
            },
            fnRowCallback: function(nRow, aData, iDisplayIndex) {
                ...
                if (aData[3]) {
                    $('td:eq(9)', nRow).html('<img title="Notificar Destinatário" src="imagens/notificar_destinatario.png" class=botao32 onclick=notificar(' + aData[0] + ');>');
                } else {
                    $('td:eq(9)', nRow).html('<img title="Este prazo não possui um destinatário específico que possa ser notificado!" src="imagens/notificar_destinatario_inativo.png" class=botao32 >');
                }

                $('td:eq(6)', nRow).html('<img title="Detalhar" src="imagens/alterar.png" class=botao32 onclick=detalharSolicitacao(' + aData[0] + ');>');
...
<table class="display" border="0" id="tabela_prazos_enviados_setor">
    <thead>
        <tr>
            <th class="style13">#</th>
    ...
    <tbody>
        <tr>
            <td colspan="5" title="test" class="dataTables_empty">Nenhum prazo enviado pelo setor!</td>
        </tr>
    </tbody>
</table>
