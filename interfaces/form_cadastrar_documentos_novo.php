...
function jquery_validar_campos_cadastrar_documentos(){
    ...
    if(
        ...
        if( $('#DIGITAL_CADASTRAR_DOCUMENTOS').val().length !== 7 ){ $('#DIGITAL_CADASTRAR_DOCUMENTOS').focus(); return null; }
        if( $('#TIPO_CADASTRAR_DOCUMENTOS').val() == '' ){ $('#TIPO_CADASTRAR_DOCUMENTOS').focus(); return null; }
        if( jQuery.trim( $('#NUMERO_CADASTRAR_DOCUMENTOS').val() ) == '' ){ $('#NUMERO_CADASTRAR_DOCUMENTOS').focus(); return null; }
        if( $('#ORIGEM_CADASTRAR_DOCUMENTOS').val() == null ){ $('#ORIGEM_CADASTRAR_DOCUMENTOS').focus(); return null; }
        if( jQuery.trim( $('#DATA_DOCUMENTO_CADASTRAR_DOCUMENTOS').val() ) == '' ){ $('#DATA_DOCUMENTO_CADASTRAR_DOCUMENTOS').focus(); return null; }
        if( $('#DESTINO_CADASTRAR_DOCUMENTOS').val() == null ){ $('#DESTINO_CADASTRAR_DOCUMENTOS').focus(); return null; }
        if( jQuery.trim( $('#TECNICO_RESPONSAVEL_CADASTRAR_DOCUMENTOS').val() ) == '' ){ $('#TECNICO_RESPONSAVEL_CADASTRAR_DOCUMENTOS').focus(); return null; }
        if( $('#ASSUNTO_CADASTRAR_DOCUMENTOS').val() == null ){ $('#ASSUNTO_CADASTRAR_DOCUMENTOS').focus(); return null; }
        if( $('#PROCEDENCIA_CADASTRAR_DOCUMENTOS').val() == '' ){ $('#PROCEDENCIA_CADASTRAR_DOCUMENTOS').focus(); return null; }
        if( $('#PROCEDENCIA_CADASTRAR_DOCUMENTOS').val()=='E' ){
            if( $('#DATA_ENTRADA_CADASTRAR_DOCUMENTOS').val() == '' ){ $('#DATA_ENTRADA_CADASTRAR_DOCUMENTOS').focus(); return null; }
            if( jQuery.trim( $('#RECEBIDO_POR_CADASTRAR_DOCUMENTOS').val() ) == null ){ $('#RECEBIDO_POR_CADASTRAR_DOCUMENTOS').focus(); return null; }
        }
