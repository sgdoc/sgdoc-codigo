...
<script type="text/javascript">
    ...
    /*Funcoes*/
    function jquery_receber_documentos(digital) {
        try {
            var c = confirm('Você tem certeza que deseja receber este(s) documento(s)?\n[' + digital + ']\nObs:Todos documentos vinculados também serão recebidos!');
            if (c) {
                $("#progressbar").show();
    ...
    $(document).ready(function() {
        ...
        oTableDocumentos = $('#TabelaDocumentos').dataTable({
            ...
            sAjaxSource: "modelos/documentos/listar_documentos_caixa_entrada.php",
            oLanguage: {
...
?>
        ...
        /*Botoes Recebimento*/
        $('#botao-receber-documentos').click(function() {
            if (digital_selecionados(total_documentos).length > 0) {
                jquery_receber_documentos(digital_selecionados(total_documentos));
            } else {
                alert('Nenhum documento esta selecionado!');
            }
        });
