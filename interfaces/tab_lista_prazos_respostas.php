...
<script type="text/javascript">
    ...
    var oTableRespostas = null;

    $(document).ready(function() {
        $('#tabs-prazos-respostas').click(function() {
            oTableRespostas.fnDraw(false);
        });

        /*DataTable*/
        oTableRespostas = $('#tabela_prazos_respostas').dataTable({
            ...
            sAjaxSource: "modelos/prazos/listar_prazos_respostas.php",
            oLanguage: {
            ...
            fnServerData: function(sSource, aoData, fnCallback) {
                $.getJSON(sSource, aoData, function(json) {
                    ...
                    if (json.iTotalRecords == 0) {
                        jquery_datatable_complementa_mensagem_vazia('tabela_prazos_respostas');
                    }
                });
            },
            fnRowCallback: function(nRow, aData, iDisplayIndex) {
                ...
                $('td:eq(7)', nRow).html('<img title="Detalhar Solicitação" src="imagens/alterar.png" class=botao32 onclick=detalharSolicitacao(' + aData[0] + ');>');

                $('td:eq(8)', nRow).html('<img title="Detalhar Resposta" src="imagens/alterar.png" class=botao32 onclick=detalharResposta(' + aData[0] + ');>');

                if (aData[11] < 0) {
                    $('td', nRow).removeClass('vermelho');
                    $('td', nRow).addClass('vermelho');
                    $('td:eq(11)', nRow).html('Respondido ' + (-1 * aData[11]) + ' dia(s) depois do prazo.');
                } else if (aData[11] == 0) {
                    $('td', nRow).removeClass('verde');
                    $('td', nRow).addClass('verde');
                    $('td:eq(11)', nRow).html('Respondido no dia que acabou o prazo.');
                } else if (aData[11] > 0) {
                    $('td', nRow).removeClass('verde');
                    $('td', nRow).addClass('verde');
                    $('td:eq(11)', nRow).html('Respondido ' + (aData[11]) + ' dia(s) antes do prazo.');
                }
                return nRow;
            }
        });
...
<table class="display" border="0" id="tabela_prazos_respostas">
    <thead>
        <tr>
            <th class="style13">#</th>
