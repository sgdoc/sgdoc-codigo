$.fn.simpleTree = function(opt){
    return this.each(function(){
        ...
        TREE.setAjaxNodes = function(node, parentId, callback){
            if($.inArray(parentId,ajaxCache) == -1){
                ...
                var data = eval('('+$.trim($('>li', node).text())+')');
                ...
                if(data.url){
                    $.ajax({
                        ...
                        success: function(responce){
                            ...
                            node.attr("idElemento", data.idElemento);
                            node.attr("stAusente", data.stAusente);
                            node.html(responce);
                            $.extend(node,{
                                url:data.url
                            });
        ...
        TREE.setTreeNodes = function(obj, useParent, updateMode){
            ...
            // available
                ...
                if(TREE.option.drag)
                {
                    ...
                    $("<img>").attr({
                        id : "tree_plus",
                        src : "plugins/tree/css/images/plus.gif"
                    }).css({
                        width: "7px",
                        display: "block",
                        position: "absolute",
        ...
        TREE.setTrigger = function(node){
            $('>span',node).before('<img class="trigger" src="plugins/tree/css/images/spacer.gif" border=0>');
        ...
        TREE.dragStart = function(event){
            ...
            if(mousePressed)
            {
                ...
                if(event.target.tagName.toLowerCase()=='span' && $.inArray(event.target.className, Array('text','active','trigger'))!= -1)
                {
                    ...
                    {
                        if(isrc.indexOf('minus')!=-1)$("#tree_plus").attr('src','plugins/tree/css/images/plus.gif');
                        $("#tree_plus").css({
                            "left": screenScroll.x,
                            "top": screenScroll.y
                        }).show();
                    ...
                    {
                        mouseMoved = false;
                        $("#tree_plus").attr('src','plugins/tree/css/images/minus.gif');
                        $("#tree_plus").css({
                            "left": screenScroll.x,
                            "top": screenScroll.y
                        }).show();
   ...
   */
                        TREE.setAjaxNodes(ajaxChild,parent.id, function(){
                            parent.className = parent.className.replace('close','open');
                            mouseMoved = true;
                            $("#tree_plus").attr('src','plugins/tree/css/images/plus.gif');
                            $("#tree_plus").css({
                                "left": screenScroll.x,
                                "top": screenScroll.y
                            }).show();
